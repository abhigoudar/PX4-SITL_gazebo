...
#include "gazebo_imu_plugin_ros2.h"
...
void GazeboImuPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->ros_node_ = rclcpp::Node::make_shared("gazeob_imu_plugin_ros");
  ...
  imu_pub_ros_ = ros_node_->create_publisher<sensor_msgs::msg::Imu>("/" + model_->GetName() + imu_topic_, 
...
void GazeboImuPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  imu_msg_ros_.angular_velocity.x = angular_velocity_I[0];
  imu_msg_ros_.angular_velocity.y = angular_velocity_I[1];
  imu_msg_ros_.angular_velocity.z = angular_velocity_I[2];
  // TODO: Fix this
  double acc_cov = std::pow(imu_parameters_.accelerometer_noise_density,2);
  double ang_cov = std::pow(imu_parameters_.gyroscope_noise_density,2);
  imu_msg_ros_.linear_acceleration_covariance[0] = acc_cov;
  imu_msg_ros_.linear_acceleration_covariance[4] = acc_cov;
  imu_msg_ros_.linear_acceleration_covariance[8] = acc_cov;
  imu_msg_ros_.angular_velocity_covariance[0] = ang_cov;
  imu_msg_ros_.angular_velocity_covariance[4] = ang_cov;
  imu_msg_ros_.angular_velocity_covariance[8] = ang_cov;
  //
  imu_pub_ros_->publish(imu_msg_ros_);
}
